...
class setup_MMS:
    ...
    def broken_L2_norm(self, u, uh, subdomains, degree_rise=4):
        """ contains norm for evaluating error on gamma_mesh for
        I_M by calculating the error on each side of the intracellular mesh """
        ...
        for subd_i, u_i in zip(subdomains, u):
            ...
            # Pick an edge
            CompiledSubDomain(subd_i).mark(cell_f, 1)
            ...
            # And the error there
            error_i = errornorm(u_i, uh_i, 'L2', degree_rise=degree_rise)
            error += error_i**2
